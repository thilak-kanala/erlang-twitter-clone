...
-export([send_tweet_testing/3]).
-export([send_retweet_testing/3]).
-export([demo_live_user_testing/3]).
-include("utility.hrl").

start(NNodes, _State, _Bridge) ->
    engine:start().
    ...
    % engine:update_all_nodes(NNodes),

    % engine:show_loopdata(_State, _Bridge).

send_tweet_testing(NNodes, _State, _Bridge) ->
    Tweet1 = get_random_tweet(1, NNodes),
    engine:send_tweet_from_Nth_Node(1, Tweet1),
    engine:update_all_nodes(NNodes),
...
send_retweet_testing(NNodes, _State, _Bridge) ->
    node:query_mention_tweets(3),
    timer:sleep(500),
    engine:update_all_nodes(NNodes),
    engine:send_random_retweet_from_Nth_Node(3),
    timer:sleep(500),
    engine:show_loopdata(_State, _Bridge).

demo_live_user_testing(_NNodes, _State, _Bridge) ->
    engine:start(),
    timer:sleep(500),
    _AllUsers = register_all_users(5, 5, []),
    LiveUser = {'\"live_user\":', {'\"valid\":', true}, {'\"user\":',2}, {'\"hashtags\":', ["#hashtag5"]}},
    engine:register_live_user(LiveUser),
    
    Tweet1 = get_random_tweet(1, 5),
    engine:send_tweet_from_Nth_Node(1, Tweet1),

    Tweet2 = get_random_tweet(3, 5),
    engine:send_tweet_from_Nth_Node(3, Tweet2),

    Tweet3 = get_random_tweet(4, 5),
    engine:send_tweet_from_Nth_Node(4, Tweet3),
...
simulate_zipf_distribution(NNodes) ->
    ...
    _StringUserSubscriberCount = io_lib:format("~p",[_UserSubscriberCount]),
    StringUserSubscriberCount = lists:flatten(_StringUserSubscriberCount),

    _StringUserSubscriberCount2 = string:replace(StringUserSubscriberCount, "'", "", all),
    StringUserSubscriberCount2 = lists:flatten(_StringUserSubscriberCount2),

    _StringUserSubscriberCount3 = string:replace(StringUserSubscriberCount2, ":,", ": ", all),
    StringUserSubscriberCount3 = lists:flatten(_StringUserSubscriberCount3),

    file:write_file("./scratch.txt", io_lib:fwrite("~s", [StringUserSubscriberCount3])).

    % file:write_file("./scratch.txt", io_lib:fwrite("~w~n~n~w~n~n~w~n~n", [_NRanks, _NSubs, _NTweets])).
...
register_all_users(0, _NNodes, AllUsers) ->
    ...
    _StringAllUsers = io_lib:format("~p",[AllUsers]),
    StringAllUsers1 = lists:flatten(_StringAllUsers),

    _StringLoopData2 = string:replace(StringAllUsers1, "'", "", all),
    StringLoopData2 = lists:flatten(_StringLoopData2),

    _StringLoopData3 = string:replace(StringLoopData2, ":,", ": ", all),
    StringLoopData3 = lists:flatten(_StringLoopData3),

    file:write_file("./scratch.txt", io_lib:fwrite("~s", [StringLoopData3])),
    AllUsers;
